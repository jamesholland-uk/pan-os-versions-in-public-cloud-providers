...
- [AWS](aws/README.md) - including listings of AMI IDs
- [Azure](azure/README.md) - with Offers and SKUs
- [GCP](gcp/README.md) - with image names
...
- **AWS regions.** AMI IDs are collected from every region the AWS account behind this project has enabled. The AWS workflow opts the account in to new regions as AWS launches them, which takes a few hours to complete; any region not covered yet is listed at the bottom of [the AWS listing](aws/README.md) so the gap is visible rather than silent. An AMI missing for a region here does not mean Palo Alto Networks has not published there.
- **End-of-life dates.** Held in [`scripts/eol.json`](scripts/eol.json), maintained by hand from Palo Alto Networks' [end-of-life summary](https://www.paloaltonetworks.com/services/support/end-of-life-announcements/end-of-life-summary). Trains with no date recorded are left unmarked.
...
- **New listings are flagged, not guessed.** An AWS marketplace listing from Palo Alto Networks that isn't in `scripts/aws-processing.py`'s product-code table is logged as a warning on every run, so a new one gets noticed instead of quietly going unpublished.
...
python scripts/aws-processing.py                                # needs AWS credentials
AZURE_SUBSCRIPTION_ID=... python scripts/azure-processing.py    # az login, or a service principal
...
python scripts/gcp-processing.py gcp-list.txt gcp-rama-list.txt gcp-airs-list.txt

python -m unittest discover -s scripts -p "test_*.py"           # parsing and EOL tests
```

Each script writes its own Markdown page and JSON file, and refuses to publish an empty listing if the API returns nothing. The three clouds each encode PAN-OS versions differently; `scripts/panos_version.py` holds the parsers and is the only place that logic lives.
...
## Support
Community supported, as per the [support statement](.github/SUPPORT.md).
